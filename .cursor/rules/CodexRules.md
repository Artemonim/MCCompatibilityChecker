...
Дополнительный временный набор правил:
- Не запускай терминал в background - ты не сможешь получить к нему доступ после запуска.
- При работе с Python используй `.venv/Scripts/Activate.ps1 && {то, что ты хотел запустить в консоли}` в первом вызове консоли за сессию, т.к. консоль не умеет автоматически активировать venv.
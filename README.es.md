...
- **Pester** (módulo de PowerShell, necesario para las pruebas de `checker.ps1` si no se usa `-NoPester`)
...
Actualmente incluye los patrones `Something broke...`, `Something went wrong...` y `Что-то сломалось...`. Para nuevos idiomas, basta con añadir esta lista al archivo de locale correspondiente.
...
| `-OutcomeTimeoutSeconds <sec>` | Tiempo de espera del resultado tras pulsar Play (por defecto: 60) |
...
.\checker.ps1 -NoPester   # Omitir pruebas de Pester
.\checker.ps1 .\scripts\Shared-FileOps.ps1  # Verificar solo el archivo/ruta indicado
```
...
- La etapa de Recuperación es experimental, pero está activada por defecto (`[Stages].EnableRecovery=true`)

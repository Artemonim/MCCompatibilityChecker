...
1. **Базовый анализ лога** — читает краш-лог, ищет кандидатов в тексте ошибки и изолирует их в dependency-priority порядке
...
3. **Аддитивное наслоение** — убирает все моды, оставляет core-библиотеки, добавляет остальные слоями (по уровням зависимостей, экспоненциальными батчами). При вылете батча — triage и изоляция внутри батча
4. **Гибридная субтрактивная изоляция** — fallback: dependency-aware уровни, экспоненциальные/бинарные пробы на ранних уровнях и линейная изоляция на поздних
...
│   ├── Isolate-Incompatible-Mod.ps1     # Гибридная субтрактивная изоляция (fallback)

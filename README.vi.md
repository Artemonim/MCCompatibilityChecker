...
- **Pester** (module PowerShell, cần cho test của `checker.ps1` nếu không dùng `-NoPester`)
...
| `EnableRecovery` | Kích hoạt khôi phục các thủ phạm ảo (mặc định: `true`) |
...
Hiện đã có các mẫu tiêu đề `Something broke...`, `Something went wrong...` và `Что-то сломалось...`. Đối với các ngôn ngữ mới, chỉ cần thêm danh sách này vào tệp ngôn ngữ tương ứng.
...
| `-OutcomeTimeoutSeconds <sec>` | Thời gian chờ kết quả sau khi nhấn Play (mặc định: 60) |
...
.\checker.ps1 -NoPester   # Bỏ qua test Pester
.\checker.ps1 .\scripts\Shared-FileOps.ps1  # Chỉ kiểm tra tệp/đường dẫn được chỉ định
```
...
- Giai đoạn Recovery hiện vẫn đang là thử nghiệm, nhưng bật theo mặc định (`[Stages].EnableRecovery=true`)

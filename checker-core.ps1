function Invoke-McccCheckerCore {
  ...
  $sharedLocalizationPath = Join-Path -Path $RepoRoot -ChildPath "scripts\Shared-Localization.ps1"
  ...
  Initialize-McccLocalization -StartDir $RepoRoot | Out-Null
  ...
  if ($hadAnalyzerError) { return $ExitCodeOnErrors }
  if ($localeValidationFailed -or $pesterValidationFailed -or $findings.Count -gt 0) { return $ExitCodeOnFindings }
  return 0
}
